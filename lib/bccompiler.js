...
var bccompiler = (function() {
	...
	Compiler.prototype.compileKeywordExpression = function(block, lexer)
	{
		...
		{
			case 'ndo:':
				block.compileByteCode(this.machine.ByteCodes.NativeDo);
				return;
			case 'nat:':
				block.compileByteCode(this.machine.ByteCodes.NativeAt);
				return;
			case 'nat:put:':
				block.compileByteCode(this.machine.ByteCodes.NativeAtPut);
				return;
			case 'napply:':
				block.compileByteCode(this.machine.ByteCodes.GetNull);
				block.compileByteCode(this.machine.ByteCodes.NativeApply);
				return;
			case 'napply:with:':
				block.compileByteCode(this.machine.ByteCodes.NativeApply);
				return;
			case 'nnew:':
				block.compileByteCode(this.machine.ByteCodes.NativeNew);
				return;
			case 'ifTrue:':
				block.compileByteCode(this.machine.ByteCodes.IfTrue);
				return;
			case 'ifTrue:ifFalse:':
				block.compileByteCode(this.machine.ByteCodes.IfTrueIfFalse);
				return;
			case 'ifFalse:':
				block.compileByteCode(this.machine.ByteCodes.IfFalse);
				return;
			case 'ifFalse:ifTrue:':
				block.compileByteCode(this.machine.ByteCodes.IfFalseIfTrue);
				return;
		}

        var mthselector = selector.replace(/:/g,'_');
		var position = block.addValue(mthselector);
		block.compileByteCode(this.machine.ByteCodes.GetValue, position);
		block.compileByteCode(this.machine.ByteCodes.SendMessage, arity);
	}
	...
	Compiler.prototype.compileBinaryExpression = function(block, lexer)
	{
		...
		{			
			...
			{
				case '+':
					block.compileByteCode(this.machine.ByteCodes.Add);
					break;
				case '-':
					block.compileByteCode(this.machine.ByteCodes.Subtract);
					break;
				case '*':
					block.compileByteCode(this.machine.ByteCodes.Multiply);
					break;
				case '/':
					block.compileByteCode(this.machine.ByteCodes.Divide);
					break;
				case ',':
					block.compileByteCode(this.machine.ByteCodes.Concatenate);
					break;
				case '=':
					block.compileByteCode(this.machine.ByteCodes.Equal);
					break;
				case '==':
					block.compileByteCode(this.machine.ByteCodes.StrictEqual);
					break;
				case '~=':
					block.compileByteCode(this.machine.ByteCodes.NotEqual);
					break;
				case '<':
					block.compileByteCode(this.machine.ByteCodes.Less);
					break;
				case '>':
					block.compileByteCode(this.machine.ByteCodes.Greater);
					break;
				case '<=':
					block.compileByteCode(this.machine.ByteCodes.LessEqual);
					break;
				case '>=':
					block.compileByteCode(this.machine.ByteCodes.GreaterEqual);
					break;
				default:
					if (isNumber(token))
					{
						...
						block.compileByteCode(this.machine.ByteCodes.GetValue, position);
						block.compileByteCode(this.machine.ByteCodes.SendMessage, 1);
					}
					break;
			};
	...
	Compiler.prototype.compileUnaryExpression = function(block, lexer)
	{
		...
		while (token != null && token.type === TokenType.Name)
		{
			...
			{
				case 'yourself':
					block.compileByteCode(this.machine.ByteCodes.GetYourself);
					break;
				case 'nnew':
					block.compileByteCode(this.machine.ByteCodes.GetNull);
					block.compileByteCode(this.machine.ByteCodes.NativeNew);
				...
				default:
					var position = block.addValue(token.value);
					block.compileByteCode(this.machine.ByteCodes.GetValue, position);
					block.compileByteCode(this.machine.ByteCodes.SendMessage, 0);
					break;
			}
	...
	Compiler.prototype.compileTerm = function(block, lexer)
	{
		...
		{
			this.target = null;
			block.compileByteCode(this.machine.ByteCodes.LastTarget);
			return;
		}
		...
		if (token.type === TokenType.Name) 
		{
            if (token.value == "self")
            {
                block.compileByteCode(this.machine.ByteCodes.GetSelf);
                return;
            }
            
            if (token.value == "super")
            {
                block.compileByteCode(this.machine.ByteCodes.GetSuper);
                return;
            }
            ...
            {
                block.compileByteCode(this.machine.ByteCodes.GetFalse);
                return;
            }
            
            if (token.value == "true")
            {
                block.compileByteCode(this.machine.ByteCodes.GetTrue);
                return;
            }
			...
			if (isargument || token == null || token.type !== TokenType.Sign || token.value != ':=')
			{
				...
				if (isparameter)
					block.compileByteCode(this.machine.ByteCodes.GetParameter, position);
				else if (isargument)
					block.compileByteCode(this.machine.ByteCodes.GetArgument, position);
				else if (islocal)
					block.compileByteCode(this.machine.ByteCodes.GetLocal, position);
				else if (isvariable)
					block.compileByteCode(this.machine.ByteCodes.GetInstanceVariable, position);
				else
					block.compileByteCode(this.machine.ByteCodes.GetGlobalVariable, position);
			...
			if (islocal)
				block.compileByteCode(this.machine.ByteCodes.SetLocal, position);
			else if (isvariable)
				block.compileByteCode(this.machine.ByteCodes.SetInstanceVariable, position);
			else
				block.compileByteCode(this.machine.ByteCodes.SetGlobalVariable, position);
		...
		if (token.type == TokenType.Punctuation && token.value == '{')
		{
			...
			block.compileByteCode(this.machine.ByteCodes.NewList, nexprs);
			return;
		}
		...
		if (token.type === TokenType.Punctuation && token.value == '[')
		{
			...
			block.compileByteCode(this.machine.ByteCodes.GetBlock, position);
			return;
		}
		...
		{
			this.compileExpression(block, lexer);
			block.compileByteCode(this.machine.ByteCodes.Return);
			return;
		}
		...
		if (token.type == TokenType.Sign && token.value == '<')
		{
			...
			{
				block.compileByteCode(this.machine.ByteCodes.NativePrimitive, block.addValue(token.value));
			...
			block.compileByteCode(this.machine.ByteCodes.Primitive, parseInt(token.value));
			return;
		}
	...
	Compiler.prototype.compileConstantArray = function(block, lexer)
	{
		...
		block.compileByteCode(this.machine.ByteCodes.NewList, nelements);
	}
