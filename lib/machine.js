...
var machine = (function() {
	...
	function ExecutionBlock(block, self, args, locals)
	{
		this.block = block;
		this.self = self;
		this.args = args;
    ...
    ExecutionBlock.prototype.toFunction = function () {
        var exeblock = this;
		...
		while (ip < l)
		{
			...
			switch(bytecode) {
				...
				case ByteCodes.GetBlock:
					var nv = bc[ip++];
					stack.push(new ExecutionBlock(this.block.values[nv], this.self, null, this.locals));
					break;
				...
				case ByteCodes.NativeNew:
					...
					stack.push(target.prototype.constructor.apply(newobj, args));
