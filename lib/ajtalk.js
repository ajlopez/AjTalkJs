...
(function(exports, top) {
		...
		while (token != null && token.isBinaryOperator())
		{
			...
			switch (token.value) 
			{
				...
				default:
					...
					block.compileByteCode(ByteCodes.GetValue, position);
					block.compileByteCode(ByteCodes.SendMessage, 2);
					break;
			};
	...
	Compiler.prototype.compileTerm = function(block, lexer)
	{
		...
		if (token.isString() || token.isNumber() || token.isSymbol())
		{
			var position = block.addValue(token.value);
			block.compileByteCode(ByteCodes.GetValue, position);
			return;
	...
	function ChunckReader(text)
	{
		var position = 0;
		
		this.nextChunck = function() {
	...
	exports.ChunckReader = ChunckReader;
